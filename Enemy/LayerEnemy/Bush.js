...
export class Bush {
    constructor(x, y, data) {
        ...
        this.sprite.src = config.enemies.bush.sprite.src;
        ...
        this.sprite.onerror = () => {
            console.warn(`Failed to load sprite: ${config.enemies.bush.sprite.src}`);
            this.spriteLoaded = false;
        };

        this.curFrame = 0;
        this.lastFrameTime = 0;
        this.animationSpeed = config.enemies.bush.sprite.animationSpeed;
    }
    ...
    updateAnimation() {
        ...
        if (curTime - this.lastFrameTime > this.animationSpeed) {
            this.curFrame = (this.curFrame + 1) % config.enemies.bush.sprite.frames;
    ...
    draw(ctx) {
        if (this.spriteLoaded) {
            const spriteConfig = config.enemies.bush.sprite;
