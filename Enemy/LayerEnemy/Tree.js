...
export class Tree {
    constructor(x, y, data) {
        ...
        this.sprite.src = config.enemies.tree.sprite.src;
        ...
        this.sprite.onerror = () => {
            console.warn(`Failed to load sprite: ${config.enemies.tree.sprite.src}`);
            this.spriteLoaded = false;
        };

        this.curFrame = 0;
        this.lastFrameTime = 0;
        this.animationSpeed = config.enemies.tree.sprite.animationSpeed;
    }
    ...
    updateAnimation() {
        ...
        if (curTime - this.lastFrameTime > this.animationSpeed) {
            this.curFrame = (this.curFrame + 1) % config.enemies.tree.sprite.frames;
    ...
    draw(ctx) {
        if (this.spriteLoaded) {
            const spriteConfig = config.enemies.tree.sprite;
