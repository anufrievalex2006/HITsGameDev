...
export class SpeedUp {
    constructor(x, y, data) {
        ...
        this.sprite.src = config.collectible.speedUp.sprite.src;
        ...
        this.sprite.onerror = () => {
            console.warn(`Failed to load sprite: ${config.collectible.speedUp.sprite.src}`);
            this.spriteLoaded = false;
        };

        this.curFrame = 0;
        this.lastFrameTime = 0;
        this.animationSpeed = config.collectible.speedUp.animationSpeed;
    }
    ...
    updateAnimation() {
        ...
        if (curTime - this.lastFrameTime > this.animationSpeed) {
            this.curFrame = (this.curFrame + 1) % config.collectible.speedUp.sprite.frames;
    ...
    draw(ctx) {
        if (this.spriteLoaded) {
            const spriteConfig = config.collectible.speedUp.sprite;
